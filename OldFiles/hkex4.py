...
# https://github.com/ericlx/crawlerForShareHolding/blob/master/hkex4.py
__author__ = 'EricLX'
    ...
    while len(date) < 8:
        ...
        if int(date) >= int(getDateOfToday()):
            ...
            with open('shareholding.csv', 'a') as f:
                f.write('\n')
                f.write('Date,' + date[-2:] + '/' + date[4:6] + '/' + date[:4])
                f.write('\n')
                for key in myList2:
...
action = input("Do you need information for another date(if yes, enter 'Y', else 'N'?): ")
